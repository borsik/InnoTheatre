<?php
	...
	$query = "SELECT DISTINCT per.name, pl.play_date, pl.play_time, per.description, pl.price, t.name,
					pl.sid
					FROM play pl
	...
	while($row = pg_fetch_row($result)){
		...
		$query2 = "SELECT DISTINCT ppl.name, ppl.surname
					FROM play pl
					INNER JOIN performance per ON per.per_id = pl.per_id
					INNER JOIN theater t ON t.tid = pl.tid
					LEFT OUTER JOIN Perf_prod pp ON pp.per_id = per.per_id
					LEFT OUTER JOIN Person ppl ON ppl.pid = pp.pid
					WHERE pl.sid = ".$row[6]."";
					$result2 = pg_query($db_theatres, $query2);
					
					$return = $return.'
					<p class="card-text"> (';
					while($row2 = pg_fetch_row($result2)){
					$return = $return.$row2[0].' '.$row2[1].', ';
					}
					$return = $return.') </p>
